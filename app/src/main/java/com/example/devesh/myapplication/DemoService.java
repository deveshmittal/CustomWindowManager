 ...
 */
public class DemoService extends Service implements ShoppingWebSiteList.ShoppingWebSiteListListener {
    //
    ...
    private RelativeLayout 				mContentContainerLayout;// Contains everything other than buttons and info
    private RelativeLayout 				mLogoLayout;			// Contains  logo
    private RelativeLayout mCoverLayout;
    private RelativeLayout mCoverHelperLayout;// . This helps with fade animations.
    private LinearLayout mPlayerButtonsLayout;	// Contains playback buttons
    private LinearLayout mInfoLayout;		// Contains Text information on the current item
    ...
    private ShoppingWebSiteList mPlaylist;
    ...
    public void onCreate() {
        ...
        mRootLayout= (RelativeLayout)LayoutInflater.from(this).inflate(R.layout.service_demo, null);
        ...
        mPlayerButtonsLayout = (LinearLayout) LayoutInflater.from(this).
                inflate(R.layout.viewgroup_buttons, null);
        mRootLayout.addView(mPlayerButtonsLayout);

        mInfoLayout = (LinearLayout) LayoutInflater.from(this).
                inflate(R.layout.viewgroup_info, null);
        mRootLayout.addView(mInfoLayout);
        ...
        mPlayButton = (ImageButton) mPlayerButtonsLayout.findViewById(R.id.button_play);
        mPauseButton = (ImageButton) mPlayerButtonsLayout.findViewById(R.id.button_pause);
        mTitleView = (TextView) mInfoLayout.findViewById(R.id.item_name);
        mTypeView = (TextView) mInfoLayout.findViewById(R.id.type_name);

        mPlaylist = new ShoppingWebSiteList(this);
        ...
        mRootLayout.postDelayed(new Runnable() {
            ...
            public void run() {
                ...
                is=null;
                try {
                    is = getAssets().open(mPlaylist.getCurrentInfo().mCoverPath);
                } catch (IOException e) {
                    e.printStackTrace();
                }
                ...
                params.setMargins(
                        ...
                        marg*3,
                        marg);
                mRootLayout.updateViewLayout(mInfoLayout, params);
                mTitleView.setText(mPlaylist.getCurrentInfo().mTitle);
                mTypeView.setText(mPlaylist.getCurrentInfo().mType);
    ...
    private void animateButtons(){
        ...
        if (mRootLayoutParams.x < -mRootLayout.getWidth()/TRAY_HIDDEN_FRACTION){
            ...
            mInfoLayout.startAnimation(animations);
            mCoverLayout.startAnimation(animationAlpha);
        }else{
            ...
            mCoverLayout.clearAnimation();
        }
    }

    // Load new cover image
    private void changeDisplayInfo(){
        ...
        // Load new bitmap
        Bitmap bmap = Utils.loadMaskedBitmap(is, mCoverLayout.getHeight(), mCoverLayout.getWidth());

        // Change backgrounds
        mCoverHelperLayout.setBackgroundDrawable(mCoverLayout.getBackground());
        mCoverLayout.setBackgroundDrawable(new BitmapDrawable(getResources(), bmap));
        ...
        mCoverHelperLayout.startAnimation(fadeOutAnim);
        mCoverLayout.startAnimation(fadeInAnim);
    ...
    // Mock callback - Notifies the UI about progress.
    @Override
    public void updateProgress(int playheadPosition) {
        // TODO - Will be implemented later
    }

    // Mock  playlist callback - Notifies the UI to update info. Current has changed.
    @Override
    public void startedNext() {
        changeDisplayInfo();
    }

    // Mock  playlist callback - Provides the UI thread handler so that the callee thread could update UI
    @Override
